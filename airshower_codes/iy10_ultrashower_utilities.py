...
def GetAnglesDet( evtImageXZ, evtImageYZ ):
  ...
  for angle in peakAngles:
    if ( abs(  0 - abs(angle) ) < 1 ): continue
    if ( abs( 90 - abs(angle) ) < 1 ): continue
    goodAngles.append(angle)

  if len(goodAngles) == 0: 
    return (0, 0, 0, 0)
  
  goodAngles = numpy.array(goodAngles).reshape(-1, 1)
  print("goodAngles YZ = ", goodAngles)
  dbscan = DBSCAN(eps=5, min_samples=2)
  labels = dbscan.fit_predict(goodAngles)
  dbsAng = goodAngles[labels >= 0]
  if (len(goodAngles) == 1): 
    dbsAng = goodAngles
  #if (len(goodAngles) >= 2 and len(dbsAng) == 0): dbsAng = numpy.array([0])
  print("dbsAng= ", dbsAng)
  if (dbsAng.size == 0):
    return (0, 0, 0, 0)
  aveAngleYZ = numpy.average( dbsAng )
  stdAngleYZ = numpy.std( dbsAng )
  ...
  if trkAngleYZ > 0:
    for angle in goodAngles:
      AngleXZ.append(angle)  
  else:
    for angle in goodAngles:
      angle = 180 + angle
      AngleXZ.append(angle)

  AngleXZ = numpy.array(AngleXZ).reshape(-1, 1)
  print("AngleXZ= ", AngleXZ)

  dbscan = DBSCAN(eps=5, min_samples=2)
  labels = dbscan.fit_predict(AngleXZ)
  dbsAng = AngleXZ[labels >= 0]
  print("dbsAng= ", dbsAng)
  if (len(AngleXZ) == 1): 
    dbsAng = AngleXZ
  #if (len(AngleXZ) >= 2 and len(dbsAng) == 0): dbsAng = numpy.array([0])
  if (dbsAng.size == 0):
    return (0, 0, 0, 0)
  trkAngleXZ = numpy.average( dbsAng )
  stdAngleXZ = numpy.std( dbsAng )
